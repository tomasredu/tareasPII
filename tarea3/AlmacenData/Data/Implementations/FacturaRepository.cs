...
namespace Almacen.Data.Implementations
{
    public class FacturaRepository : IFacturaRepository
    {
        ...
        {
            using (var command = _connection.CreateCommandTransaction())
            {
                ...
                try
                {
                    ...
                    foreach (var d in factura.Detalles)
                    {
                        ...
                        {
                            cmdDet.Transaction = _connection.GetTransaction();
        ...
        internal IEnumerable<Factura> GetByFiltro(int? idForma, DateTime? fecha)
        {
            using (var command = _connection.CreateCommand())
            {
                //texto y parametros del command
                command.CommandText = "SP_RECUPERAR_FACTURAS_FILTRO";
                command.CommandType = CommandType.StoredProcedure;

                List<SqlParameter> parameters = new List<SqlParameter>();

                if (!fecha.HasValue)
                    parameters.Add(new SqlParameter("@fecha", DBNull.Value));
                else
                    parameters.Add(new SqlParameter("@fecha", fecha));

                if (!idForma.HasValue)
                    parameters.Add(new SqlParameter("@forma", DBNull.Value));
                else
                    parameters.Add(new SqlParameter("@forma", idForma));
